...
export default function EngagementTab(props: EngagementTabProps) {
  ...
  const resolvedMomentumStatus = React.useMemo<MomentumStatus | null>(() => {
    if (defenseResolution) {
      return defenseResolution.status;
    }

    return baseMomentumStatus;
  }, [baseMomentumStatus, defenseResolution]);
    ...
    <section className="dc-engagement" aria-label="Engagement Overview">
      ...
      <article className="dc-engagement-card dc-momentum-card">
        <header className="dc-momentum-header">
          ...
          {resolvedMomentumStatus ? (
            <span
              className={`dc-momentum-status dc-momentum-status--${resolvedMomentumStatus.tone}`}
              role="status"
              aria-live="polite"
            >
              {resolvedMomentumStatus.label}
            </span>
          ) : null}
